...
class Language::SX {
    ...
    has default_libraries => (
        ...
        required    => 1,
        coerce      => 1,
        default     => sub { require Language::SX::Library::Core; [Language::SX::Library::Core->new] },
        handles     => {
    ...
    has '+_trait_namespace' => (
        default     => 'Language::SX::Trait',
    );

    method _build_reader {

        require Language::SX::Reader;
        return Language::SX::Reader->new(
...
Reads a L<Language::SX::Document> from a C<$source> definition.
...
List of default library objects for new documents. Defaults to L<Language::SX::Library::Core>.

@attr document_traits
Traits that should be applied to new documents.

@attr include_path
Where to look for other C<Language::SX> files to load.
...
C<Language::SX> is an implementation for executing S-Expression code in Perl. The used language is
...
Lisp and with this also Scheme are list-based languages. C<Language::SX> implements more of a data
...
Includes a simple rendering system with L<Language::SX::Renderer::Plain> that allows specially formed
...
First the body is turned into a L<stream|Language::SX::Reader::Stream> by the
L<reader|Language::SX::Reader>.

=item *

The reader will then create a new L<document|Language::SX::Document>. This new document receives the previously 
created stream and inflates itself into a tree structure.

=item *

To compile the tree structure, the document will create an L<inflator|Language::SX::Inflator>. The purpose of
...
See L<Language::SX::Library::Quoting> for details on how to build complex inline data
structures.

=head2 Context

Every call in C<Language::SX> is made in scalar context. If you really have to call 
something in list context, see L<Language::SX::Library::Data::Functions/"apply/list">
for information.
...
Regular expressions have a slightly different syntax in L<Language::SX> than in raw
...
See L<Language::SX::Library::Data::Regex> for details about regular expression
handling.
...
Lists in a C<Language::SX> context refers to array references. Since there are no 
non-reference arrays in this system, and I didn't want to write "array reference" all
the time, I use both interchangably.

You can create lists with L<Language::SX::Library::Data::Lists/list> or by 
L<quoting|Language::SX::Library::Quoting> a C<()> or C<[]> structure.
...
will return a list containing the object, since L<list|Language::SX::Library::Data::Lists/list>
...
See L<Language::SX::Library::Types> for more information.
...
There are two kinds of comments in C<Language::SX>. The first is a simple line based comment like in
...
You can L<import|Language::SX::Library::Inserts/import> values that are exported
...
libraries are organized under the default L<Language::SX::Library::Core> group.

=head2 Variables

!TAG<variables>

See L<Language::SX::Library::ScopeHandling> for information on how variables can be
...
=item * L<SourceType|Language::SX::Types/SourceType> C<$type>
...
Returns the meta object for C<Language::SX> as an instance of L<Class::MOP::Class::Immutable::Moose::Meta::Class>.
