...
our $SX = Language::SX->new(document_traits => [qw( CompileTidy )], default_libraries => [], use_global_cache => 1);
...
sub bareword {
    my ($val) = @_;
    require Language::SX::Runtime::Bareword;
    return Language::SX::Runtime::Bareword->new(value => $val);
}
